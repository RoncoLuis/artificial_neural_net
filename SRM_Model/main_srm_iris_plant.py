...
def test_phase(obj_srm, padres, error_padres, x_data_test, y_data_test):
    error_min = np.argmin(error_padres)

    y_predicted = obj_srm.srm_train_test(x_data=x_data_test, y_data=y_data_test, \
...
for i in range(35):
    ...
    acc_m1,cm_m1 = test_phase(obj_srm=obj_srm,padres=nuevos_pesos,error_padres=error,x_data_test=x_test,y_data_test=y_test)
    # =================== Microexperimento 2 ===================
    new_weights,error_n = train_phase(obj_srm=obj_srm,obj_ev_dif=obj_ev_dif,x_data=x_test,y_data=y_test)
    acc_m2,cm_m2 = test_phase(obj_srm=obj_srm,padres=new_weights,error_padres=error_n,x_data_test=x_train,y_data_test=y_train)
